...
**Date:** 2026-08-26
**Status:** APPROVED (Tiered Autonomy) — **REV2 implemented through Phase 5 (2026-08-31). Phase 6 Discord NotificationSink DEFERRED AGAIN by user 2026-08-31. Do not implement Phase 6 until an explicit command.**
**Protocol:** Incremental Phase Review & Delegation Protocol (`.agents/workflows/phase-review-protocol.md`) — @Reviewer sign-off required after EVERY phase; no phase proceeds without explicit user command.
...
**Implementation branch:** `feat/ai_kb`.

**Phase status as of 2026-08-31**

| Phase | Scope | Status |
|---|---|---|
| 0 | AI Pulse enrichment | **Implemented** — tests green |
| 1 | CalibrationService + silent channels | **Implemented** — contract tests green |
| 2 | Frontend CALIBRATING UX + freeze | **Implemented** — Vite build clean |
| 3 | Tier A/B + Session Guardian | **Implemented** — 3 reviewer P1s still open (see Open follow-ups) |
| 4 | KB health audit + recency backfill + UTC 4h + warm-start | **Implemented** — review P1s (parse/300s overlay/collision-before-write) fixed |
| 5 | Strictness profiles + alignment + drift detector (A4, M8) | **Implemented** — tests green, Vite build clean |
| 6 | Discord `NotificationSink` | **DEFERRED AGAIN** (2026-08-31). Socket.IO remains the only sink. |
...
| EX-3 | `_emit_trade_entry` emitting `trade_entry` (:298, :364) — **but this is NOT the only leak surface** (see REV1 contract: `trade_result`, `status_update`, `notification`, pulse pending/abort also leak) | `trade_service.py:226–233, 189–196` | Location of `emit_trade_channel` helper — 🔧 Extend (Phase 1, C1 amendment) | ✅ Phase 1 |
| EX-4 | ~~`_session_trades` deque + append at :360~~ **Persistent truth is `TradeService.repository.write_trade` → session JSONL (`trade_service.py:287,363`)**; `_session_trades` is only the in-memory AI Pulse cache. Also: `report_outcome` increments `_session_trade_count` **unconditionally at :352 — voids are counted** | `auto_ghost.py:191,352,360`; `trade_service.py:287,363` | Tag `TradeRecord.entry_context` BEFORE `write_trade`; budget counts settled win/loss only | ✅ Phase 1 |
| EX-5 | AI Pulse abort/pending emissions (`ai_pulse_aborted` ×12, `ai_pulse_pending`) | `auto_ghost.py:887–1104` | Route to `calibration_*` variants during mode | ✅ Phase 1 |
...
| EX-7 | `RuntimeStrategyConfigRequest` (~48 fields) + `update_runtime_settings` forwarding | `strategy.py:16–63`, streaming forward map | Add 4 calibration config fields through same pipeline | ✅ Phase 1 |
| EX-8 | AI Pulse prompt pipeline `_run_ai_pulse_insight` | `streaming.py:872–1087` | Phase 0 enrichment target; Phase 3 milestone/final report emitter | ✅ Phase 0 / 3 |
| EX-9 | **PAYOUT GAP:** asset summaries omit payout % despite `_payout_cache`/`_resolve_asset_payout_pct` in same class (:464) | `streaming.py:912–929` | Critical — AI cannot reason about `minimum_payout_pct` gate today | ✅ Phase 0 |
| EX-10 | Regex fallback fabricates `confidence: 85` | `streaming.py:1038,1053` | Same fabrication class as M5 payout bug already fixed | ✅ Phase 0 |
| EX-11 | Prompt omits Bayesian WP cache, HTF verdicts, volatility/liquidity readings, trajectory attribution, rolling WR | `streaming.py:898–989` | 6 additional blindness fixes | ✅ Phase 0 |
...
| EX-14 | Historical corpus: 135 session JSONs (~13,400×60s + 1,005×300s), priors READY per horizon; 1,029 KB patterns (stale ~2.5 months) | `app/data/ghost_trades/`, KB json | Phase 4 recency-weighted backfill source | ✅ Phase 4 backfill path exists (staging-only) |
| EX-15 | Named Ghost Protocol presets via `loadGhostProtocol` — **⚠ was z-score + regime only; Phase 5 expanded to vol/liq/Bayesian/payout/amount/concurrency (M8)** | `useSettingsStore.js` `applyGhostProtocolGates` / `GHOST_STRICTNESS_PRESETS` | Relaxed/Conservative/Strict Apply cards | ✅ Phase 5 |
| EX-16 | Developer Mode toggle | settings store / TopBar | Surface for DEV_SUGGESTION observations | ✅ Reuse |
| EX-17 | Notification sink = Socket.IO only; no webhook code anywhere | repo-wide grep confirmed | Greenfield for Discord (Phase 6) behind `NotificationSink` | ⏸ Deferred again 2026-08-31 |
| EX-18 | **Drawdown is a 300s cooldown, NOT a kill-switch** — `:407` sets `_drawdown_cooldown_until`; `_session_halted` is read (`:336,502`) but **never set anywhere** (dead flag) | `auto_ghost.py:406–409` | CalibrationService must OWN the kill-switch → ABORTED after every settlement; do NOT reuse drawdown cooldown (C4) | ✅ Phase 1 |
| EX-19 | **App.jsx 400ms debounced settings sync POSTs `/api/strategy/runtime-config` on every change AND on mount; `update_config` wipes the session when `enabled and (not previous_enabled or not _session_id)`** (`auto_ghost.py:233–254`) | `App.jsx:301–306` | Runtime-config writes MUST be refused (409) while calibration RUNNING — otherwise frontend sync overwrites the preset mid-run and can wipe the sample (C3 — repeats 2026-06-16 failure mode) | ✅ Phase 1 |
| EX-20 | Fabricated `confidence: 85` exists in THREE sites: regex fallbacks `:1038, :1053` **and the system prompt JSON example `:964`** | `streaming.py:964,1038,1053` | All three must be fixed in Phase 0 — fixing only the regex leaves the model instructed to emit 85 (M7) | ✅ Phase 0 |
| EX-21 | `_AUTO_GHOST_FIELD_SPECS["bayesian_min_probability"]` = `(float, None, None)` — **no bounds** on the direct `update_config` path (API path is bounded `ge=0.50, le=0.90` at `strategy.py:58`); a model emitting `53.5` (percent form) would set an impossible floor | `auto_ghost.py:135` | Clamp to `(float, 0.50, 0.90)` in Phase 0 — production footgun even without calibration (M6) | ✅ Phase 0 |
...
### Phase 0 — [x] AI Pulse Enrichment Quick Wins (independently valuable) — IMPLEMENTED & REVIEWER-SIGNED-OFF 2026-08-26 (109/109 tests green)
...
### Phase 1 — [x] Backend Core: CalibrationService + Silent Channels (REV1) — IMPLEMENTED 2026-08-26; Phase 1.1 patches + drain/restore contracts green
...
### Phase 2 — [x] Frontend: Calibration UX + Visual Suppression — IMPLEMENTED 2026-08-26 (`CalibrationPanel.jsx`, freeze on GhostSettings/TopBar/AI Tools, journal Calibration filter). Vite build clean. Live browser calibration run still outstanding.
...
### Phase 3 — [x] AI Autonomy Engine (Tier A/B) + Milestone Reports + Session Guardian — IMPLEMENTED 2026-08-26 (`calibration_autonomy.py` + Guardian loop). Reviewer P1s remain open (see Open follow-ups). A3 suggestion-effectiveness ledger is incomplete.
...
### Phase 4 — [x] KB Health Audit + Historical Backfill (+ REV2 AI-Decision Optimizers) — IMPLEMENTED 2026-08-30/31. Reviewer P1s fixed: canonical `generated_utc` parse, 300s recency overlay commit, collision guard before write. Staging-only backfill; UTC 4h origin 22:00; recency half-life 21d; warm-start for Guardian.
...
### Phase 5 — [x] Strictness Profiles + Condition-Alignment Notifier — IMPLEMENTED 2026-08-31 (`ghost_protocol_profiles.py`, M8 `applyGhostProtocolGates`, CalibrationPanel Apply cards, Guardian alignment + A4 drift). Tests: `test_ghost_protocol_profiles.py`. Vite build clean. No live-browser Apply-card pass.
...
### Phase 6 — [ ] Discord Notification Sink — **DEFERRED AGAIN 2026-08-31 (user command). Do not start.**
- `NotificationSink` interface; Socket.IO remains sink #1 (Phase 5 alignment/drift/Guardian notifications already emit on Socket.IO). Discord webhook sink #2 (~30 lines, no bot required) is out of scope until an explicit command.
- User-configurable webhook URL in settings. Same abstraction later carries signal broadcasting.

---

## Verification Checklist

- [x] Phase 0: prompt-contract test asserts payout/WP/HTF/volatility/liquidity/attribution/rolling-WR sections present; no fabricated `confidence` values **in regex output OR prompt example (`:964`)**; `bayesian_min_probability` spec bounds `(0.50, 0.90)` enforced (53.5-form rejected/clamped); payout suggestions in percent
- [x] Phase 1 (REV1 — contract-test battery): `test_calibration_no_live_ui_leak` (all 5 surfaces), payout units, 409 runtime lock, drawdown abort, session_id isolated, settings restore, in-flight drain, budget counts settled only, 24-trade/25-min + 4-trade checkpoints + 12-trade milestones
- [x] Phase 3 (REV2 — Session Guardian): N≥20 propose-only + warm-start divergence flag implemented. **Open:** A3 suggestion-effectiveness ledger incomplete; three reviewer P1s (see Open follow-ups)
- [x] Phase 4 (REV2): backfill staged-only; UTC-hour 4h-block in schema + loader (A1); recency decay 90d vs 10d <50% at 21d half-life (A2); warm-start baseline for Guardian (D7). Reviewer P1s (parse / 300s overlay / collision-before-write) fixed 2026-08-31
- [x] Phase 5 (REV2): drift detector fires on synthetic OOD stream (A4); protocol schema expansion loads all gate families (M8)
- [x] `conda run -n QuFLX-v2 python -m pytest test_ghost_protocol_profiles.py test_kb_health_phase4.py test_calibration_contracts.py test_calibration_autonomy.py test_preflight_gate_contracts.py test_auto_ghost.py -q` → 73 passed (2026-08-31). Vite `npm --prefix app/frontend run build` clean
- [ ] Phase 6 Discord NotificationSink — **deferred again; do not start**
- [ ] Manual: live paper calibration run — badge progress correct; UI shows NO trades, NO sounds, NO pending cards during run; copy-mode execute blocked; journal shows tagged session after (excluded by default); milestone AI Pulse reports arrive; Tier A changelog entries present in session file; pre-calibration settings restored on DONE; Apply cards for Relaxed/Conservative/Strict
- [x] No behavioral regressions in existing pre-flight gates covered by `test_preflight_gate_contracts.py` + `test_auto_ghost.py`

### Open follow-ups (not Phase 6)

Do **not** treat these as “Phase 6”. They are leftover from Phases 3–5.

1. **Phase 3 P1s (reviewer, skipped when user advanced to Phase 4):** `_finalize` awaits AI milestone review before D4 restore (can hang PROPOSING); Tier B rejects `amount`/`expiration` if they sit in `locked_fields`; Guardian `allowed_regimes` remaining-list can be `[]` which AutoGhost treats as allow-all.
2. **A3 suggestion-effectiveness ledger** (pre/post 20-trade WR delta) is not fully wired into AI prompts.
3. **Phase 4 suggestions (non-blocking):** live Bayesian scoring still uses unweighted integers (recency overlay is storage/commit); corrupt warm-start disables D7 with only a log; staging-modal copy still talks about integer prior merges for recency backfill.
4. **Live browser calibration pass** has not been run.
5. **Phase 6** Discord `NotificationSink` — deferred again 2026-08-31.

## Files Touched Summary

| Phase | Files |
|---|---|
| 0 | `app/backend/services/streaming.py`, `test_ai_pulse_prompt.py` |
| 1 | `calibration_service.py`, `auto_ghost.py`, `trade_service.py`, `strategy.py`, `streaming.py`, `test_calibration_contracts.py` |
| 2 | `CalibrationPanel.jsx`, `useCalibrationStore.js`, `GhostTradingWidget.jsx`, `JournalView.jsx`, `GhostSettings.jsx`, `TopBar.jsx`, `useSettingsStore.js`, `App.jsx` |
| 3 | `calibration_autonomy.py`, `calibration_service.py` (Guardian), `auto_ghost.py` (tier tables) |
| 4 | `kb_health.py`, `scripts/kb_health_backfill.py`, `scripts/analyze_trade_intelligence.py`, `shared/utc_time_blocks.py`, `shared/bayesian_prior_store.py`, `journal_stats_service.py`, `KnowledgeBaseStagingModal.jsx`, `test_kb_health_phase4.py` |
| 5 | `ghost_protocol_profiles.py`, `useSettingsStore.js` (`applyGhostProtocolGates` / `GHOST_STRICTNESS_PRESETS`), `CalibrationPanel.jsx` Apply cards, Guardian alignment+drift, `test_ghost_protocol_profiles.py` |
| 6 | **Not started.** Would be `notification_sink.py` + settings webhook field. |
