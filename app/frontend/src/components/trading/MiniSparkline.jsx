...
export default function MiniSparkline({ ticks, className = '' }) {
  ...
  const gradientId = `miniSparklineLine-${instanceId}`;
  
  const { trend, positive } = useMemo(() => {
    const t = getTrendPercent(series);
    return { trend: t, positive: t >= 0 };
  }, [series]);
  ...
  return (
    <div className={`relative h-16 overflow-hidden rounded-xl border border-white/5 bg-[#1a1717] ${className}`}>
      <svg className="h-full w-full" viewBox="0 0 240 64" aria-hidden="true">
        ...
        {path && <path d={path} fill="none" stroke={`url(#${gradientId})`} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />}
