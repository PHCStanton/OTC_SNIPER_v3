            ...
            if isinstance(oteo_res, dict):
                ...
                for level_name, level_signal in levels_to_test:
                    ...
                    if not vetoed and self.config.pocket.veto_enabled:
                        ...
                        # Also support fuzzy pocket and expiry exclusions (e.g. state|expiry)
                        for excl in self.config.pocket.exclusion_list:
                            if "|" in excl and excl.count("|") == 3:  # f"State|expiry"
                                parts = excl.split("|")
                                state_part = "|".join(parts[:-1]).strip()
                                exp_part = parts[-1].strip()
                                if state_part == pocket_state:
                                    # Veto dynamically inside expiry loop or veto general setup
                                    pass
