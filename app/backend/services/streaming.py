...
_AUTO_GHOST_FORWARD_MAP: dict[str, str] = {
    ...
    "auto_ghost_min_zscore_enabled": "min_zscore_enabled",
    "auto_ghost_min_zscore": "min_zscore",
    "auto_ghost_max_zscore_enabled": "max_zscore_enabled",
    "auto_ghost_max_zscore": "max_zscore",
...
class StreamingService:
    ...
    def __init__(self, sio_server=None):
        ...
        # C1 fix: bounded per-asset recent-tick buffers feeding the AI Pulse pre-flight gate
        self._recent_ticks: Dict[str, Deque[Dict[str, Any]]] = {}
        # C2 fix: latest Bayesian win-probability cache consumed by the pre-flight gate
        self._latest_bayesian_wp: Dict[str, Dict[str, float]] = {}
    ...
    def update_runtime_settings(
        ...
        *,
        ...
        _param_values = {
            ...
            "auto_ghost_min_zscore_enabled": auto_ghost_min_zscore_enabled,
            "auto_ghost_min_zscore": auto_ghost_min_zscore,
            "auto_ghost_max_zscore_enabled": auto_ghost_max_zscore_enabled,
            "auto_ghost_max_zscore": auto_ghost_max_zscore,
            "ai_trade_interval": ai_trade_interval,
            "ai_pulse_enabled": ai_pulse_enabled,
            "ai_pulse_interval_seconds": ai_pulse_interval_seconds,
    ...
    def update_allowed_assets(self, assets: list[str]) -> None:
        ...
        for asset in removed:
            ...
            self._recent_ticks.pop(asset, None)
            self._latest_bayesian_wp.pop(asset, None)
            if self._ai_review:
    ...
    def stop(self) -> None:
        ...
        self._latest_bayesian_wp.clear()
    ...
    def _get_or_create_engines(self, asset: str) -> tuple[OTEO, MarketContextEngine, ManipulationDetector]:
        if asset not in self._oteo_engines:
            ...
            # Seed the pre-flight tick-flow buffer so the HTF confluence gate has data immediately
            seed_buffer: Deque[Dict[str, Any]] = deque(maxlen=600)
            for tick in valid_ticks:
                seed_buffer.append({"t": float(tick["t"]), "p": float(tick["p"])})
            self._recent_ticks[asset] = seed_buffer
            if valid_ticks:
                for tick in valid_ticks:
    ...
    async def _process_tick_inner(self, asset: str, price: float, timestamp: float, source: str) -> None:
        ...
        # C1 fix: maintain the bounded recent-tick buffer consumed by the pre-flight gate
        tick_buffer = self._recent_ticks.get(asset)
        if tick_buffer is None:
            tick_buffer = deque(maxlen=600)
            self._recent_ticks[asset] = tick_buffer
        tick_buffer.append({"t": timestamp, "p": price})
        ...
        if is_warmed_up:
            ...
            # C2 fix: cache latest Bayesian win probabilities emitted by the signal-filter extension
            _wp60 = enriched_result.get("bayesian_win_probability_60s")
            if _wp60 is not None:
                try:
                    self._latest_bayesian_wp[asset] = {
                        "bayesian_win_probability": float(_wp60),
                        "bayesian_win_probability_60s": float(_wp60),
                        "bayesian_win_probability_300s": float(
                            enriched_result.get("bayesian_win_probability_300s") or _wp60
                        ),
                    }
                except (TypeError, ValueError):
                    logger.warning("Invalid Bayesian win probability for %s: %r", asset, _wp60)
    ...
    def _get_asset_market_context_snapshot(self, asset: str) -> Dict[str, Any]:
        ...
        ctx["recent_ticks"] = list(self._recent_ticks.get(asset, ()))
        _wp_cache = self._latest_bayesian_wp.get(asset)
        if _wp_cache:
