...
- **Calibration Mode (single owner):** ALL calibration state lives in `CalibrationService` (`IDLE→RUNNING→ANALYZING→PROPOSING→DONE|ABORTED`). The UI only observes. Silence is backend event routing (`calibration_*`), not frontend suppression. Runtime-config writes 409 while locked. D4 restores the pre-run snapshot including `None`. Master KB writes go through staging only. Phase 6 Discord NotificationSink is deferred; Socket.IO is the only sink.
- **Ghost Protocol presets (M8):** `loadGhostProtocol` / `applyGhostProtocolGates` must load z-score, regime, vol/liq, Bayesian floor, payout, amount, and concurrency — not z-score+regime alone.
